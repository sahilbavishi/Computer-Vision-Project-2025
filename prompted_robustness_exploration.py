...
# Subject: Computer Vision Coursework. Robustness exploration on the prompted CLIP model
# Date: 31.03.2025


"""0. Preliminary code"""

print("ROBUSTNESS EXPLORATION WITH DIFFERENT PERTURBATION TYPES (PROMPTED CLIP):")
...
# Defining paths
model_dir = f'Data/Output/Models/PROMPTED_CLIP_DICE_epoch_1.pth'
images_dir = 'Data/Input/Test/color/'
masks_dir = 'Data/Input/Test/label/'
heatmaps_dir = 'Data/Input/Test/heatmaps/'
output_plots_dir = 'Data/Output/Visuals/Prompted_CLIP_Robustness/'
...
# Loading pre-trained prompted CLIP model
...
model = PromptedClipDecoder().to(device)
...
def evaluate_gaussian_noise(model, images_dir, masks_dir, std_dev_levels, device, save_as:str="a_gaussian_noise"):
    ...
    for std_dev in std_dev_levels:
        ...
        for img_file in image_files:
            ...
            heatmap_path = os.path.join(heatmaps_dir, img_file.split('.')[0]+'_heatmap.png')

            image = np.array(Image.open(img_path).convert("RGB").resize((500, 500)))
            ...
            heatmap = Image.open(heatmap_path).convert('L').resize((500, 500))
...
def evaluate_gaussian_blur(model, images_dir, masks_dir, max_blur_level, device, save_as:str="b_gaussian_blurring"):
    ...
    for blur_level in range(max_blur_level + 1):
        ...
        for img_file in image_files:
            ...
            blurred_image = np.array(Image.open(img_path).convert("RGB").resize((500, 500)))
...
def shift_heatmap(heatmap, distance):
    """Shifts the heatmap by a given distance in a random direction."""
    h, w = heatmap.shape
    angle = np.random.uniform(0, 2 * np.pi)  # Random angle in radians
    dx = int(distance * np.cos(angle))  # X shift
    dy = int(distance * np.sin(angle))  # Y shift
    
    translation_matrix = np.float32([[1, 0, dx], [0, 1, dy]])
    shifted_heatmap = cv2.warpAffine(heatmap, translation_matrix, (w, h), borderValue=0)
    return shifted_heatmap

def evaluate_heatmap_shift(model, images_dir, masks_dir, shift_distances, device, save_as="i_heatmap_shift"):
    """Evaluates the model with shifting heatmaps and plots Dice score vs. shift distance."""
    image_files = [f for f in os.listdir(images_dir) if f.endswith('.jpg')]
    dice_scores = []
    ...
    for distance in shift_distances:
        ...
        for img_file in image_files:
            ...
            heatmap = np.array(Image.open(heatmap_path).convert('L').resize((500, 500)))
            shifted_heatmap = shift_heatmap(heatmap, distance)
            heatmap_tensor = transforms.ToTensor()(Image.fromarray(shifted_heatmap)).unsqueeze(0).to(device)
            
            image_tensor = torch.cat([image_tensor, heatmap_tensor], dim=1)
    ...
    plt.figure()
    plt.plot(shift_distances, dice_scores, marker='o')
    plt.xlabel("Heatmap Shift (pixels)")
    plt.ylabel("Mean Dice Score")
    plt.title("Imprecise prompt")
    plt.grid()
    plt.savefig(output_plots_dir + save_as + ".pdf", format="pdf")
    
    return dice_scores

print("\ni) Imprecise prompt...", end=' ')
shift_distances = [0, 25, 50, 75, 100, 125, 150, 200, 250, 300]
dice_scores = evaluate_heatmap_shift(model, images_dir, masks_dir, shift_distances, device)
print("Done!")
print(" -"*49)
print("| heatmap shift   =", " | ".join([f"{value:{digits+2}}" for value in shift_distances]), "|")
print("| mean Dice score =", " | ".join([f"{d:{digits+2}.{digits}f}" for d in dice_scores]), "|")
print(" -"*49,"\n")