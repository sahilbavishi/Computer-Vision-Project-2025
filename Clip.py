...
# Extracting and saving CLIP features
def extract_clip_features(image_path):
    image = Image.open(image_path).convert("RGB")
    image_tensor = preprocess_val(image).unsqueeze(0).to(device)
    with torch.no_grad():
        features = model.encode_image(image_tensor)
    features = features / features.norm(p=2, dim=-1, keepdim=True) # Normalization
    return features.cpu()
...
for image_name in trainval_images:
    ...
    # Saving each feature tensor separately
...
class SegmentationDecoder(nn.Module):
    def __init__(self, clip_feature_dim=512, num_classes=classesNum):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Upsample(size=preprocessedSize, mode='bilinear', align_corners=True),
